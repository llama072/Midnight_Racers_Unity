...
namespace MidnightRacers.Settings
{
    ...
    public class SettingsUIBuilder : MonoBehaviour
    {
        ...
        // ─── Runtime state ─────────────────────────────────────────
        Canvas _canvas;
        Transform _root;            // where all UI elements are parented
        ...
        public void Build()
        {
            // Reset our own transform — guards against the builder being placed inside
            // a scaled / positioned parent (e.g. inside an existing Canvas).
            ...
            BuildScanlines(_root);

            BuildHeader(_root);
            var (categoryEntries, panelMap) = BuildBody(_root);
            var (applyBtn, resetBtn)        = BuildButtonBar(_root);
            BuildFooter(_root);
        ...
        void BuildCanvas()
        {
            // If we're already inside an existing Canvas, build into our own RectTransform —
            // do NOT nest a second Canvas (would inherit the parent's scale and break layout).
            var existingCanvas = GetComponentInParent<Canvas>();
            if (existingCanvas != null)
            {
                _canvas = existingCanvas;
                var ourRT = GetComponent<RectTransform>();
                if (ourRT == null) ourRT = gameObject.AddComponent<RectTransform>();
                ourRT.anchorMin     = Vector2.zero;
                ourRT.anchorMax     = Vector2.one;
                ourRT.offsetMin     = Vector2.zero;
                ourRT.offsetMax     = Vector2.zero;
                ourRT.localScale    = Vector3.one;
                ourRT.localPosition = Vector3.zero;
                _root = transform;
                return;
            }
